...
class ModelBase():
    def __init__(self, config):
        self.config = config                         # opt
        self.save_dir = config.model_save_dir  # save models
    ...
    def save_network(self, save_dir, network, network_label, epoch_label):
        save_filename = 'epoch_{}_{}.pth'.format(epoch_label, network_label)
    ...
    def update_E(self, decay=0.999):
        ...
        for k in netG_params.keys():
            netE_params[k].data.mul_(decay).add_(netG_params[k].data, alpha=1-decay) # EMA
