...
def augment_flow(flow, mode=0):
    if mode == 0:
        return flow
    elif mode == 1:
        flow = np.flipud(np.rot90(flow, k=1))       
        flow = np.stack((flow[..., 1], flow[..., 0]), axis=-1)
    elif mode == 2:
        flow = np.flipud(flow)          
        flow[..., 1] *= -1                    
    elif mode == 3:
        flow = np.rot90(flow, k=3)  
        flow = np.stack((-flow[..., 1], flow[..., 0]), axis=-1) 
    elif mode == 4:
        flow = np.flipud(np.rot90(flow, k=2)) 
        flow = np.stack((-flow[..., 0], flow[..., 1]), axis=-1)                        
    elif mode == 5:
        flow = np.rot90(flow, k=1) 
        flow = np.stack((flow[..., 1], -flow[..., 0]), axis=-1)  
    elif mode == 6:
        flow = np.rot90(flow, k=2)  
        flow *= -1                                
    elif mode == 7:
        flow = np.flipud(np.rot90(flow, k=3))  
        flow = np.stack((-flow[..., 1], -flow[..., 0]), axis=-1)  
...
def imresize(img, size, interpolation=cv2.INTER_LANCZOS4):
    return cv2.resize(img, (size[1], size[0]), interpolation=interpolation)
